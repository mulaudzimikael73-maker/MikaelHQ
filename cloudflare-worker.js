...
async function tg(env,m,p){try{if(!env?.TELEGRAM_BOT_TOKEN||!env?.TELEGRAM_CHAT_ID)return {ok:false,skipped:true};const r=await fetch(`https://api.telegram.org/bot${env.TELEGRAM_BOT_TOKEN}/${m}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(p)});return await r.json();}catch(e){return {ok:false,error:String(e?.message||e)};}}
...
const hqAuth=(req,env)=>!!(String(req.headers.get(HQ_KEY_HEADER)||"")&&env?.MIKAEL_HQ_KEY&&String(req.headers.get(HQ_KEY_HEADER))===String(env.MIKAEL_HQ_KEY));
const hqOnly=(req,env)=>hqAuth(req,env);
