...
/* =========================================================
   🖤 MIKAEL HQ / TWO-WAY CO-OP SYSTEM
   KV binding: LIZZY_CLAIMS
   Secret: MIKAEL_HQ_KEY
   ========================================================= */
...
const hqAuth=(req)=>!!(String(req.headers.get(HQ_KEY_HEADER)||"")&&env.MIKAEL_HQ_KEY&&String(req.headers.get(HQ_KEY_HEADER))===String(env.MIKAEL_HQ_KEY));
const hqOnly=(req)=>hqAuth(req);
...
export default{async fetch(req,env){
 ...
 if(req.method==="GET"){
   ...
   if(u.searchParams.get("action")==="escape_state"){if(!hqOnly(req))return json({success:false,error:"Unauthorized"},401);return json({success:true,state:await env.LIZZY_CLAIMS.get(HQ_ESCAPE_KEY,{type:"json"})||defaultEscape()});}
   if(u.searchParams.get("action")==="hq_activity"){if(!hqOnly(req))return json({success:false,error:"Unauthorized"},401);return json({success:true,activity:(await arrKV(env,HQ_ACTIVITY_INDEX)).slice(-100).reverse()});}
 ...
 if(b.action==="chess_move"){if(!hqOnly(req))return json({success:false,error:"Unauthorized"},401);const state={id:"chess-main",fen:S(b.fen,200),pgn:S(b.pgn,8000),turn:S(b.turn,1),status:"active",lastMove:S(b.lastMove,30),updatedAt:new Date().toISOString()};await env.LIZZY_CLAIMS.put(HQ_CHESS_KEY,JSON.stringify(state));await hqActivity(env,"♟️ Chess Move",`Mikael played ${state.lastMove||"a move"}.`,{fen:state.fen});return json({success:true,state});}
 if(b.action==="chess_reset"){if(!hqOnly(req))return json({success:false,error:"Unauthorized"},401);const state=defaultChess();await env.LIZZY_CLAIMS.put(HQ_CHESS_KEY,JSON.stringify(state));await saveArr(env,HQ_CHESS_HELP_INDEX,[]);await hqActivity(env,"♟️ Chess Reset","A new co-op chess game was started.");return json({success:true,state});}
 ...
 if(b.action==="resolve_chess_help"){if(!hqOnly(req))return json({success:false,error:"Unauthorized"},401);const lid=S(b.id,120),r=await env.LIZZY_CLAIMS.get(`hq:chesshelp:${lid}`,{type:"json"});if(r){r.status="handled";await env.LIZZY_CLAIMS.put(`hq:chesshelp:${lid}`,JSON.stringify(r),{expirationTtl:86400})}return json({success:true});}
