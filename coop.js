...
async function blobMetrics(blob,transcript=""){
 ...
 const duration=Math.max(0,(Date.now()-rapRecordStart)/1000);
...
function startRapPolling(){clearInterval(rapPollTimer);rapPollTimer=setInterval(()=>{const tab=$("rap");if(tab&&!tab.classList.contains("hidden"))loadRap()},3000)}
startRapPolling();
