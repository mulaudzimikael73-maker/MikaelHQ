...
async function hqBlobMetrics(blob,transcript=""){const ab=await blob.arrayBuffer(),ctx=new(window.AudioContext||window.webkitAudioContext)(),buf=await ctx.decodeAudioData(ab.slice(0));let sum=0,count=0,peak=0,silent=0,total=0;for(let c=0;c<buf.numberOfChannels;c++){const d=buf.getChannelData(c);for(let i=0;i<d.length;i++){const v=Math.abs(d[i]);sum+=v*v;count++;if(v>peak)peak=v;if(v<.015)silent++;total++}}const duration=buf.duration||0,rms=Math.sqrt(sum/Math.max(1,count)),silenceRatio=silent/Math.max(1,total),words=(String(transcript).match(/\b\w+\b/g)||[]).length;ctx.close();return{duration,rms,peak,silenceRatio,speechRate:duration?words/duration:0}}
